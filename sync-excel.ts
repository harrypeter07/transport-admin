...
async function main() {
  ...
  const excelEmployees = new Map<string, any>();
  const routeToShiftTime = new Map<string, string>(); // route -> shift time string
  const routeToRows = new Map<string, any[]>();
  ...
  // 1. Process Shifts
  const shiftCache = new Map<string, string>(); // HH:mm -> shiftId
  ...
  // wait for cabs sequentially since concurrent unique creations might race
  for (const p of cabUpdates) {
    try { await p; } catch (e) { /* ignore race condition duplicates */ }
  }
