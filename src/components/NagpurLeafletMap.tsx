...
export default function LeafletMap({
  ...
  useEffect(() => {
    ...
    const createEmployeeIcon = (gender: string, isViolation: boolean, isSelected: boolean, stopOrder?: number) => {
      ...
      return L.divIcon({
        html: `
          <div class="flex items-center justify-center w-6 h-6 rounded-full border border-white shadow-xs text-[10px] font-black font-mono text-white ${bgColor} ${ringClass}">
            ${label}
          </div>
        `,
