...
function StatCard({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col justify-between hover:shadow-sm transition-shadow">
      ...
      <span className={`text-3xl font-black tracking-tight ${color !== "slate" ? cls.split(" ")[0] : "text-slate-900"}`}>
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">Admin Operations</h1>
            <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full border ${systemStatus.cls}`}>
              {systemStatus.label}
            </span>
          </div>
          <p className="text-xs text-slate-500">{today} · {depotName}</p>
        </div>
        {/* Quick Actions */}
        <div className="flex flex-wrap gap-2">
          <Link
            href="/dashboard/admin/transport/optimization"
            className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl transition shadow-sm"
          >
            <Zap className="w-3.5 h-3.5" /> Generate Routes
          </Link>
          <Link
            href="/dashboard/admin/operations/leaves"
            className="flex items-center gap-1.5 px-3.5 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-xl transition"
          >
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
            <div>
              ...
              <span className={`text-3xl font-black mt-0.5 block ${m.totalPendingRequestsCount > 0 ? "text-red-600" : "text-slate-900"}`}>
                <AnimatedNumber value={m.totalPendingRequestsCount || 0} loading={loading} />
              </span>
            </div>
            <Link
              href="/dashboard/admin/operations/leaves"
              className="text-xs font-bold bg-slate-900 text-white hover:bg-slate-800 px-3.5 py-1.5 rounded-xl transition"
            >
              Review
            </Link>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
            <div>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Holidays</span>
            </div>
            <Link
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col justify-between shadow-xs">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Optimization Tools</span>
            <div className="space-y-1.5">
              <Link href="/dashboard/admin/transport/optimization" className="text-xs font-bold text-blue-600 hover:underline flex items-center gap-1">
                Open Optimizer <ArrowRight className="w-3 h-3" />
              </Link>
              <Link href="/dashboard/admin/analytics" className="text-xs font-bold text-indigo-600 hover:underline flex items-center gap-1">
      ...
      {metrics?.activeRoutes?.length > 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
          ...
          <ul className="divide-y divide-slate-100">
            {metrics.activeRoutes.map((route: any) => {
              ...
              return (
                <li key={route.id} className="p-5 hover:bg-slate-50 transition-colors flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div>
                    <p className="font-bold text-slate-900 text-sm flex items-center gap-2">
                      {route.cab.vehicleNumber}
                      <span className="text-slate-400 font-normal text-xs">·</span>
                      <span className="text-slate-600 font-semibold text-xs">{route.cab.driverName || "N/A"}</span>
                    </p>
                    <div className="mt-1.5 flex items-center gap-3">
                      <div className="flex-1 bg-slate-100 rounded-full h-1.5 w-32">
                        <div className="bg-blue-500 h-1.5 rounded-full transition-all" style={{ width: `${pct}%` }} />
                      </div>
                      <span className="text-[10px] text-slate-500 font-bold">{completed}/{total} stops</span>
                    </div>
                  </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-xs flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-xs text-slate-500">No optimized transport routes are currently running.</p>
          </div>
        </div>
      )}
    </div>
