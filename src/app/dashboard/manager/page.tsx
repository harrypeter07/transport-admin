    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs flex flex-col">
          ...
          <span className="text-4xl font-extrabold text-slate-900">{loading ? "-" : teamSize}</span>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs flex flex-col">
          ...
          <a href="/dashboard/manager/approvals" className="text-4xl font-extrabold text-blue-600 hover:underline">
            {loading ? "-" : pendingApprovals}
          </a>
        </div>
        
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs flex flex-col">
          <span className={`text-xs font-black uppercase tracking-widest mb-2 flex items-center gap-1 ${delayedCount > 0 ? 'text-red-500' : 'text-slate-400'}`}>
            <AlertCircle size={14} /> Delayed Today
          </span>
          <span className={`text-4xl font-extrabold ${delayedCount > 0 ? 'text-red-650' : 'text-slate-900'}`}>
          ...
          <div className="space-y-3">
            {metrics.delayedEmployees.map((stop: any) => (
              <div key={stop.id} className="bg-white p-4 rounded-xl border border-red-150 flex items-center justify-between">
                <div>
                  <p className="font-bold text-slate-900 text-sm">{stop.employee.name}</p>
                  <p className="text-xs text-slate-500 mt-0.5">Cab {stop.route.cab.vehicleNumber} • Driver: {stop.route.cab.driver?.name || "N/A"}</p>
                </div>
      ...
      {metrics?.activeRoutes?.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs">
          <h2 className="text-xs font-black text-slate-700 uppercase tracking-widest mb-4 flex items-center gap-1.5">
            <Map size={15} /> Subordinate Commute Progress
          </h2>
          <div className="space-y-4">
            {metrics.activeRoutes.map((route: any) => (
              <div key={route.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <p className="font-bold text-slate-900 text-sm">Cab {route.cab.vehicleNumber}</p>
                    <p className="text-xs text-slate-500 mt-0.5">Driver: {route.cab.driver?.name || "N/A"} • 📞 {route.cab.driver?.phone || "N/A"}</p>
                  </div>
                ...
                <div className="divide-y divide-slate-150 bg-white border border-slate-200 rounded-lg overflow-hidden">
                  {route.stops.map((stop: any) => (
                    <div key={stop.id} className="p-3 flex justify-between items-center text-xs">
                      <span className="font-bold text-slate-700">{stop.employee.name}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs flex flex-col justify-between">
          ...
          <a href="/dashboard/manager/approvals" className="w-full text-center px-4 py-3 bg-slate-900 text-white rounded-lg text-xs font-bold hover:bg-slate-800 transition block">
