...
// ── Shared: fetch employees + cabs from DB ─────────────────────────────────
async function fetchOptimizationInputs(shiftId: string, currentDateStr: string) {
 ...
 return { optEmployees, optCabs, fallbackShiftId };
}

// ── Shared: persist a list of OptimizedRoute[] to the database ──────────────
...
export async function POST(req: NextRequest) {
 ...
 // ── MODE: "ALL" — generate all 3 plans as a preview (no DB write) ────────
 if (mode === "ALL") {
 const { optEmployees, optCabs, fallbackShiftId } = await fetchOptimizationInputs(shiftId, currentDateStr);
 ...
 // ── MODE: "APPLY" — save a previously previewed plan chosen by admin ─────
 if (mode === "APPLY" && selectedStrategy && Array.isArray(previewRoutes)) {
 const { fallbackShiftId } = await fetchOptimizationInputs(shiftId, currentDateStr);
 await persistRoutes(previewRoutes, currentDateStr, fallbackShiftId, isPickup ?? true, selectedStrategy);
 return NextResponse.json({ success: true, count: previewRoutes.length });
 }

 // ── LEGACY MODE — single-strategy optimize + immediate save ──────────────
 const { optEmployees, optCabs, fallbackShiftId } = await fetchOptimizationInputs(shiftId, currentDateStr);
 ...
 await persistRoutes(optimizedRoutes, currentDateStr, fallbackShiftId, isPickup, mode);
