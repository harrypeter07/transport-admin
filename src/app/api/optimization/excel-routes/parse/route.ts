...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!fileKey || !sheetName) {
      return NextResponse.json({ error: "fileKey and sheetName are required" }, { status: 400 });
    }

    const filePath = path.join(process.cwd(), "data", "uploads", `${fileKey}.xlsx`);
    if (!fs.existsSync(filePath)) {
      return NextResponse.json({ error: `Upload file with key ${fileKey} not found` }, { status: 404 });
    }
    ...
    const dbByCode = new Map(dbEmployees.map((e) => [e.employeeCode.toLowerCase(), e]));
    ...
    const baselineRoutes = routes.map((r) => {
      const shiftTime = r.employees.find((e) => e.status === "YES")?.shiftTime || "05:00";
        ...
        .map((e, idx) => {
          ...
          return {
            ...
            employee: {
              ...
              name: e.name,
              employeeCode: matched?.employeeCode || e.empId || e.name.replace(/\s+/g, "-").toUpperCase(),
              gender: e.gender,
              x: matched?.x ?? 79.05,
              y: matched?.y ?? 21.06,
              address: e.address,
      ...
      return {
        ...
        capacity: 6,
        stops,
        totalDistance: Math.round(stops.length * 2.5 * 10) / 10,
        totalDuration: stops.length * 6,
    ...
    const finalSummary = {
      ...
      absentEmployeeCodes: employees.filter((e) => e.absent).map((e) => e.empId),
    ...
    return NextResponse.json({ error: "Failed to parse selected Excel sheet", details: message }, { status: 500 });
  }
}
