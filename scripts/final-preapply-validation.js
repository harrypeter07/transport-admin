...
for (const row of employeesData) {
	...
	if (!employeeMap.has(key)) {
		const statusVal =
			status === "YES" ? "PRESENT" : status === "NO" ? "NO_SHOW" : "LEAVE";
		employeeMap.set(key, {
			employeeName: name,
			employeeCode: empId,
			status: statusVal,
...
for (const emp of employees.slice(0, 10)) {
	console.log(
		`│ ${emp.employeeName.padEnd(35)} │ ${emp.employeeCode.padEnd(10)} │ ${emp.status.padEnd(12)} │`,
	);
}
if (employees.length > 10) {
	console.log(
		`│ ... and ${employees.length - 10} more employees${" ".repeat(20)} │            │              │`,
	);
}
console.log(
	"└─────────────────────────────────────┴────────────┴──────────────┘",
...
const vehicles = Array.from(vehicleMap.entries()).sort((a, b) =>
...
console.log(
	`✅ Verification: ${new Set(vehicleMap.keys()).size === vehicleMap.size ? "PASS" : "FAIL"} - No duplicates`,
);
...
for (const row of routesData) {
	...
	if (
		driverRaw.includes("Driver Details") ||
	...
	if (driverName.match(/^\d+$/) || driverName === "Escort" || !driverName)
		continue;
...
for (const driver of drivers) {
	console.log(
		`│ ${driver.driverName.padEnd(20)} │ ${driver.phone.padEnd(12)} │ ${driver.routes.padEnd(26)} │`,
	);
}
console.log(
	"└──────────────────────┴──────────────┴────────────────────────────┘",
...
(async () => {
	try {
		...
		console.log("\n--- EMPLOYEES ---");
		const workbookEmployeeSet = new Set(
			employees.map((e) => `${e.employeeName}|${e.employeeCode}`),
		);
		const dbEmployeeSet = new Set(
			dbEmployees.map((e) => `${e.name}|${e.employeeCode}`),
		);

		const matchedEmployees = Array.from(workbookEmployeeSet).filter((e) =>
			dbEmployeeSet.has(e),
		);
		const missingEmployees = Array.from(workbookEmployeeSet).filter(
			(e) => !dbEmployeeSet.has(e),
		);
		const extraEmployees = Array.from(dbEmployeeSet).filter(
			(e) => !workbookEmployeeSet.has(e),
		);
		...
		const matchedVehicles = Array.from(workbookVehicleSet).filter((v) =>
			dbVehicleSet.has(v),
		);
		const missingVehicles = Array.from(workbookVehicleSet).filter(
			(v) => !dbVehicleSet.has(v),
		);
		const extraVehicles = Array.from(dbVehicleSet).filter(
			(v) => !workbookVehicleSet.has(v),
		);
		...
		const workbookDriverSet = new Set(
			drivers.map((d) => `${d.driverName}|${d.phone}`),
		);
		const dbDriverSet = new Set(
			dbVehicles.map((v) => `${v.driverName}|${v.driverPhone}`),
		);

		const matchedDrivers = Array.from(workbookDriverSet).filter((d) =>
			dbDriverSet.has(d),
		);
		const missingDrivers = Array.from(workbookDriverSet).filter(
			(d) => !dbDriverSet.has(d),
		);
		const extraDrivers = Array.from(dbDriverSet).filter(
			(d) => !workbookDriverSet.has(d),
		);
		...
		console.log(
			`  Vehicles: ${wouldCreate.vehicles} (never create from workbook - SAFE)`,
		);
		console.log(`  Drivers: ${wouldCreate.drivers}`);

		console.log("\nUPDATES (existing records):");
		console.log(
			`  Employees: ${wouldUpdate.employees} (update roster status for date)`,
		);
		console.log(
			`  Vehicles: ${wouldUpdate.vehicles} (update metadata for existing)`,
		);
		console.log(`  Drivers: ${wouldUpdate.drivers} (update phone/details)`);

		console.log("\nDISABLES/REMOVES (ISSUE #2 - SAFE MODE):");
		console.log(
			`  Vehicles marked inactive: ${wouldDisable.vehicles} ✓ (SAFE - never disable)`,
		);
		console.log(
			`  Drivers marked inactive: ${wouldDisable.drivers} ✓ (SAFE - never disable)`,
		);
		...
		for (const employee of employees) {
			...
			const exactMatch = dbEmployees.find(
				(e) =>
					e.name.trim() === employee.employeeName.trim() &&
			...
			if (matchedBy === "none") {
				const codeMatch = dbEmployees.find(
			...
			if (matchedBy === "none") {
				const nameMatch = dbEmployees.find(
					(e) =>
						...
						employee.employeeName.toLowerCase().trim(),
				);
		...
		// Count by match type
		const matchStats = {
			name_code: employeeMatches.filter((e) => e.matchedBy === "name+code")
				.length,
			code: employeeMatches.filter((e) => e.matchedBy === "employeeCode")
				.length,
		...
		const matchPercentage = (
			((matchStats.name_code + matchStats.code + matchStats.name) /
				employees.length) *
			100
		).toFixed(1);
		...
		const hasHeaderDataInDrivers = drivers.some(
			(d) =>
		...
		const checks = [
			...
			},
			{
				...
				pass: drivers.every(
					(d) => !d.driverName.match(/^(DRIVER|MOB|Driver|Mob)/),
				),
			...
			},
			{
				...
				pass:
					wouldCreate.employees >= 0 &&
					wouldCreate.vehicles >= 0 &&
					wouldCreate.drivers >= 0,
		...
		const report = {
			...
			report2_vehicles: {
				...
				vehicles: vehicles.map(([v, r]) => ({ vehicle: v, routes: r })),
			},
			report3_drivers: {
			...
			report4_database_matching: {
				employees: {
					...
					extra: extraEmployees.length,
				},
				vehicles: {
					matched: matchedVehicles.length,
					missing: missingVehicles.length,
					extra: extraVehicles.length,
				},
				drivers: {
					matched: matchedDrivers.length,
					missing: missingDrivers.length,
					extra: extraDrivers.length,
				},
			},
			report4b_employee_matches: {
				breakdown: matchStats,
				all_matches: employeeMatches,
			},
			report5_apply_impact: {
				wouldCreate,
				wouldUpdate,
				wouldDisable,
				strategy: "SAFE: Never disable existing vehicles/drivers",
			},
		...
		fs.writeFileSync(
			path.join(
				__dirname,
				"../data/outputs",
				"gtpl-final-preapply-validation-phase3.json",
			),
			JSON.stringify(report, null, 2),
		);
		console.log(
			"Report saved: data/outputs/gtpl-final-preapply-validation-phase3.json\n",
		);
	} catch (error) {
