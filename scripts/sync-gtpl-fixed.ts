...
function parseDailyRosterSheet(
	...
	for (let i = 0; i < firstRow.length; i++) {
		...
		if (
			...
			empCodeCol = i;
		else if (header === "name" || header === "employee name") empNameCol = i;
	...
	data.forEach((row, idx) => {
		...
		if (
			empName.toLowerCase() === "escort" ||
			empName.toLowerCase().includes("driver details")
		) {
			return;
		}
...
async function main() {
	...
	const applyMode =
		args.includes("--apply") || process.env.GTPL_APPLY === "true";
	...
	console.log(
		`Source: Daily sheets (employee attendance) + Routes sheet (vehicles/drivers)`,
	);
	...
	console.log(
		`   ✅ Database: ${dbEmployees.length} employees, ${dbCabs.length} cabs`,
	);
	...
	for (const emp of dailyRosterEmployees) {
		...
		if (matchedDbEmployee) {
			...
			if (EXISTING_NORMALIZED.has(emp.normalizedName)) {
				console.warn(
					`⚠️  WARNING: ${emp.name} is in EXISTING list but not matched in DB!`,
				);
				employeesToUpdate.push({
					name: emp.name,
					code: emp.employeeCode || "NA",
	...
	const validationReport: ValidationReport = {
		...
		safetyChecks: {
			...
			passed: employeesToCreate.length <= 5 && 0 <= 5,
		},
	};
	...
	employeesToCreate.forEach((emp) => {
		console.log(`      - ${emp.name} (${emp.code}) [${emp.reason}]`);
	});
	...
	if (employeesToUpdate.length > 10) {
		console.log(`      ... and ${employeesToUpdate.length - 10} more`);
	}
	...
	if (employeesToMarkNoShow.length > 10) {
		console.log(`      ... and ${employeesToMarkNoShow.length - 10} more`);
	}

	console.log(`\n📦 VEHICLE & DRIVER ACTIONS:`);
	console.log(
		`   🚗 Vehicles to update: ${validationReport.vehiclesUpdated.length}`,
	);
	validationReport.vehiclesUpdated.forEach((v) => console.log(`      - ${v}`));

	console.log(
		`   👤 Drivers to update: ${validationReport.driversUpdated.length}`,
	);
	...
	for (const emp of employeesToUpdate) {
		...
		if (!dbEmp && emp.matchType === "NAME_NORMALIZED") {
			dbEmp = allDbEmployees.find(
				(e) =>
					normalizeEmployeeName(e.name) === normalizeEmployeeName(emp.name),
			);
		}
		if (!dbEmp && emp.matchType === "EMAIL") {
			// Try to find by email from the workbook
			const rosterEmp = dailyRosterEmployees.find((d) => d.name === emp.name);
        ...
        `;
				...
				console.error(
					`   ❌ Error updating ${emp.name}: ${(err as any).message}`,
				);
			}
		}
	}
	...
	for (const emp of employeesToMarkNoShow) {
        ...
        `;
				...
				console.error(
					`   ❌ Error marking ${emp.name}: ${(err as any).message}`,
				);
			}
		}
	}
	...
	for (const vehicleNum of validationReport.vehiclesUpdated) {
        ...
        `;
				...
				console.error(
					`   ❌ Error updating vehicle ${vehicleNum}: ${(err as any).message}`,
				);
			}
		}
	}
