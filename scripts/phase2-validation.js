...
async function validateGTPLData() {
	...
	if (applyMode) {
		console.log(
			"⚠️  WARNING: APPLY MODE ENABLED - This will write to database",
		);
	} else {
		...
		if (rows.length > 0) {
			...
			if (rows.length === 2) {
				...
				if (route1 === route2 && time1 === time2) {
					console.log(
						`  ✅ Assessment: LEGITIMATE DUPLICATION - Same route/shift (pickup+drop)"`,
					);
				} else if (route1 !== route2 || time1 !== time2) {
					console.log(
						`  ⚠️  Assessment: LIKELY LEGITIMATE - Different routes/times`,
					);
				}
			}
		}
	...
	for (const sheetName of dailySheets) {
		...
		console.log(
			`  Header pollution rows: ${headerRows} (${((headerRows / totalRows) * 100).toFixed(1)}%)`,
		);
	...
	console.log(
		`Total route-vehicle mappings: ${vehicleMapping.filter((m) => m.vehicle !== "N/A").length}`,
	);
	...
	try {
		...
		const vehicleMatched = Array.from(workbookVehicles).filter((v) =>
			dbCabNumbers.has(v),
		);
		const vehicleMissing = Array.from(workbookVehicles).filter(
			(v) => !dbCabNumbers.has(v),
		);
		const vehicleExtra = Array.from(dbCabNumbers).filter(
		...
		if (vehicleMissing.length > 0) {
			console.log(
				`  Missing vehicles: ${vehicleMissing.slice(0, 5).join(", ")}${vehicleMissing.length > 5 ? "..." : ""}`,
			);
		}
		...
		const driverMatched = Array.from(workbookDriverNames).filter((d) =>
			dbDriverNames.has(d),
		);
		const driverMissing = Array.from(workbookDriverNames).filter(
			(d) => !dbDriverNames.has(d),
		);
		const driverExtra = Array.from(dbDriverNames).filter(
			(d) => !workbookDriverNames.has(d),
		);
		...
		if (driverMissing.length > 0) {
			console.log(
				`  Missing drivers: ${Array.from(driverMissing).slice(0, 5).join(", ")}${driverMissing.length > 5 ? "..." : ""}`,
			);
		}
	} catch (err) {
	...
	const headerPollutionDetected = Object.values(headerPollutionReport).some(
		(report) => report.headerRows > 0,
	);

	console.log(`\nSafety Checks:`);
	console.log(`  APPLY MODE enabled: ${applyMode ? "✅ YES" : "❌ NO"}`);
	console.log(
		`  Vehicle count >= 1: ${vehicleCount > 0 ? "✅ YES (" + vehicleCount + ")" : "❌ NO"}`,
	);
	console.log(
		`  Employee count >= 50: ${employeeCount >= 50 ? "✅ YES (" + employeeCount + ")" : "❌ NO"}`,
	);
	console.log(
		`  Header pollution detected: ${headerPollutionDetected ? "⚠️  YES - Must fix" : "✅ NO"}`,
	);

	console.log(`\nAbort Conditions:`);
	const shouldAbort =
		vehicleCount === 0 || employeeCount < 50 || headerPollutionDetected;
	...
	if (employeeCount < 50) {
		...
		console.log(
			`  ✅ Employee count check passed (${employeeCount} employees)`,
		);
	}
	...
	if (shouldAbort) {
		...
		if (vehicleCount === 0)
			console.log("  1. Vehicle extraction failed (0 vehicles found)");
		if (employeeCount < 50)
			console.log(`  2. Insufficient employees (${employeeCount}, need 50+)`);
		if (headerPollutionDetected)
			console.log("  3. Header pollution still present in daily sheets");
	} else {
		console.log("\n✅ VALIDATION PASSED");
		if (applyMode) {
			console.log(
				"⚠️  APPLY MODE IS ENABLED - Database writes will occur on next sync",
			);
		} else {
