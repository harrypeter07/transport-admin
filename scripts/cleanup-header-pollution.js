...
const backupPath = path.join(
	__dirname,
	"../data",
	"GTPL Cab Sheet June 26 (BACKUP BEFORE CLEANUP).xlsx",
);
...
for (const sheetName of dailySheets) {
	...
	const cleanedData = data.filter((row) => {
		...
		const isHeaderPollution =
			name === "Name" || empId === "Emp ID" || email === "E mail ID";
	...
	console.log(
		`   ${sheetName}: Removed ${headerRowsInSheet} header rows (${data.length} → ${cleanedData.length})`,
	);
}
