...
async function main() {
	...
	console.log(
		`  All employees: ${Array.from(employeeSet.keys()).join(", ").substring(0, 150)}...`,
	);
	...
	const matched = Array.from(workbookEmployeeSet).filter((e) =>
		dbEmployeeSet.has(e),
	);
	const missing = Array.from(workbookEmployeeSet).filter(
	...
	if (missing.length !== 22) {
		...
		console.log(
			`   This suggests the database has changed since last validation run.\n`,
		);
	}
	...
	missingArray.forEach((emp, idx) => {
		...
		console.log(
			`   reason: Neither name nor code exists in database with matching pair\n`,
		);
	});
	...
	missingArray.forEach((emp) => {
		...
		const codeExists = dbEmployees.find(
			(e) => e.employeeCode === code && e.name !== name,
		);
		if (codeExists) {
			console.log(`⚠️  CODE MATCH: "${name}" (${code})`);
			console.log(
				`    DB has: "${codeExists.name}" (${codeExists.employeeCode})\n`,
			);
			codeOnlyMatches++;
		}

		// Check if name exists in DB (name-only match, case-insensitive)
		const nameExists = dbEmployees.find(
			(e) =>
		...
		if (nameExists) {
			...
			console.log(
				`    DB has: "${nameExists.name}" (${nameExists.employeeCode})\n`,
			);
			nameOnlyMatches++;
		}
	});
	...
	console.log(
		`This explains the discrepancy with 47 + 1 + 17 + 4 = 69 total employees.\n`,
	);
...
Match rate:                 ${((matched.length / employees.length) * 100).toFixed(1)}%
  `);
	...
	if (missing.length === 22) {
		...
		console.log(
			`   Current state shows: ${missing.length} creates, ${matched.length} updates`,
		);
	}
