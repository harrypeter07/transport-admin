...
async function main() {
  ...
  employees16.forEach((emp16, key16) => {
    ...
    employees12.forEach((emp12, key12) => {
      if (normalizeEmployeeName(emp16.name) === normalizeEmployeeName(emp12.name)) {
  ...
  console.log(`Genuinely NEW on 16-6-26 (not in 12-6-26): ${genuinelyNew.length}\n`);
  ...
  employees16.forEach((emp16) => {
    ...
    const exactMatch = dbEmployees.find(
      e => e.employeeCode === emp16.code && normalizeEmployeeName(e.name) === normalized16
    );
    ...
    const nameMatch = dbEmployees.find(e => normalizeEmployeeName(e.name) === normalized16);
    if (nameMatch) {
      result.UPDATE_EMPLOYEES.push({
        workbookName: emp16.name,
        workbookCode: emp16.code,
    ...
    const isGenuinelyNew = genuinelyNew.find(n => normalizeEmployeeName(n.name) === normalized16);
    if (isGenuinelyNew) {
  ...
  dbEmployees.forEach((dbEmp) => {
    const foundIn16 = Array.from(employees16.values()).find(
      e => normalizeEmployeeName(e.name) === normalizeEmployeeName(dbEmp.name)
    );
    if (!foundIn16) {
      absent.push({
        dbId: dbEmp.id,
  ...
  function extractVehicles(data) {
    ...
    data.forEach((row) => {
      ...
      const potentialVehicle = (row['Driver Details'] || row['Cab Number'])?.toString().trim();
      if (potentialVehicle && vehicleRegex.test(potentialVehicle)) {
        if (!vehicles.has(potentialVehicle)) {
          vehicles.set(potentialVehicle, []);
        }
        const route = row['Rout No']?.toString().trim() || row['Route No']?.toString().trim();
  ...
  function normalizeDriver(driverRaw) {
    ...
    if (!name || name === 'Driver Details' || name === 'Contact No' || /^\d{10}$/.test(name)) {
      return '';
    }
  ...
  function extractDrivers(data) {
    ...
    data.forEach((row) => {
      ...
      if (driverRaw && driverRaw !== 'Driver Details' && !driverRaw.includes('Contact')) {
  ...
  result.UPDATE_EMPLOYEES.slice(0, 10).forEach((emp) => {
    console.log(`  ✓ ${emp.workbookName} (${emp.workbookCode || 'NA'}) → ${emp.dbName} (${emp.dbCode || 'NA'}) [${emp.matchType}]`);
  });
  ...
  console.log(`✅ Employee creations (${result.CREATE_EMPLOYEES.length} <= 5): ${checks.employeeCreations ? 'PASS' : 'FAIL'}`);
  console.log(`✅ Code collisions (${codeCollisions.length} <= 2): ${checks.codeCollisions ? 'PASS' : 'FAIL'}`);
  ...
  if (report.READY_FOR_APPLY) {
    ...
    console.log('Next step: Review the report and run: npm run sync:gtpl -- --apply');
  } else {
